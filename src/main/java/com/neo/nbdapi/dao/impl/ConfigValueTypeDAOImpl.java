...
public class ConfigValueTypeDAOImpl implements ConfigValueTypeDAO {
        ...
        try (Connection connection = ds.getConnection()) {
            ...
            while (resultSet.next()) {
                comboBox = ComboBox.builder().id(resultSet.getLong("value_type_id"))
                        .text(resultSet.getString("value_type_code")+"-"+ resultSet.getString("value_type_name")+"-"+ resultSet.getString("code")).build();
    ...
    public StationValueTypeSpatialDTO getStationValueTypeSpatial(Long idStation, Long idValueType, String code) throws SQLException {
        ...
        try (Connection connection = ds.getConnection()) {
            String sql = "select c.id, c.station_id,s.station_code, s.station_name , c.value_type_id,v.value_type_code, v.value_type_name, c.variable_spatial, c.code from config_value_types c inner join stations s on s.station_id = c.station_id inner join value_types v on v.value_type_id = c.value_type_id where c.station_id = ? and c.value_type_id = ? and c.code = ?";
            PreparedStatement statement = connection.prepareStatement(sql);
    ...
    public DefaultResponseDTO createConfigValuetype(ConfigValueTypeDTO configValueTypeDTO) throws Exception {
        try (Connection connection = ds.getConnection()) {
            ...
            String sqlInsertConfig = "insert into config_value_types (ID,STATION_ID,VALUE_TYPE_ID,MIN,MAX,VARIABLE_TIME,VARIABLE_SPATIAL,START_APPLY_DATE,END_APPLY_DATE,CODE) values(config_value_types_seq.nextval,?,?,?,?,?,?,?,?,?)";
    ...
    public List<StationValueTypeSpatialDTO> getStationValueTypeSpatials(Long idConfigValueTypeParent) throws SQLException {
        ...
        try (Connection connection = ds.getConnection()) {
            String sql = "select ct.id, s.station_id, s.station_code, s.station_name, v.value_type_id, v.value_type_code, v.value_type_name, ct.variable_spatial, ct.code  from config_stations_commrelate c inner join config_value_types ct on ct.id = c.config_value_types_id inner join stations s on ct.station_id = s.station_id inner join value_types v on v.value_type_id = ct.value_type_id where c.config_value_types_parent  = ?";
            PreparedStatement statement = connection.prepareStatement(sql);
