	...
	public UserInfo findUserInfoByUsername(String username) throws SQLException {
		...
		try (Connection connection = ds.getConnection()) {
			...
			while (resultSet.next()) {
				userInfo = UserInfo.builder().id(resultSet.getString(1)).password(resultSet.getString(2)).build();
			}
			if (userInfo == null)
	...
	public UserAndMenuDTO findMenuAndApiUrlOfUser(String username, String ...args) throws SQLException {
		try (Connection connection = ds.getConnection()) {
			...
			while (resultSetMenuAcc.next()) {
				MenuDTO menuItem = MenuDTO.builder().id(resultSetMenuAcc.getInt(1))
						.menuUrl(resultSetMenuAcc.getString(2)).build();
			...
			while (resultSetApiUrl.next()) {
				ApiUrlDTO apiUrlDTO = ApiUrlDTO.builder().url(resultSetApiUrl.getString(1))
						.method(resultSetApiUrl.getString(2)).build();
				apiUrlList.add(apiUrlDTO);
			}

			UserAndMenuDTO userAndMenuDTO = UserAndMenuDTO.builder().userId(username).password(args[0])
					.menus(menuDTOList).urlApi(apiUrlList).build();
